...
public class CopyOfImportSmd extends ByteReader {
	...
	public CopyOfImportSmd(AbstractLoader loader, DrzMesh mesh) {
		this.loader = loader;
	 ...
	 * @throws IOException
	 */
	public synchronized boolean loadSmb(InputStream inputStream) throws IOException {
		...
		if (MaterialNum > 0) {
			System.out.println("Error while reading SMB file. MaterialNum(" + MaterialNum + ") must be 0.");
			return false;
		}

		// check number bones.
		if (boneNum < 1) {
			System.out.println("Error while reading SMB file. No Bone Defintion.");
			return false;
		}
		...
		for (int i = 0; i < boneNum; i++) {
			...
			if (mesh.mAnimation.mSubAnimationNum > 128) {
				System.out.println("to many bones(" + mesh.mAnimation.mSubAnimationNum + ") max (128)  in smb file.");
				return false;
			}
            ...
            for (int sa = 0; sa < mesh.mAnimation.mSubAnimationNum; sa++) //max 128
            {
                ...
                if (RotKeyNum > 0)
                {
                    ...
                    ParseRotationAniBlock(rotbuf, node, RotKeyNum, mRotMatOffset, mesh.mAnimation.mSubAnimtion[0].mAnimNode[i]);
                }
                ...
                if (PosKeyNum > 0)
                {
                    ...
                    ParsePositionAniBlock(posbuf, node, PosKeyNum, mPosMatOffset, mesh.mAnimation.mSubAnimtion[0].mAnimNode[i]);
                }
                ...
                if (SclKeyNum > 0)
                {
                    ...
                    ParseScaleAniBlock(sclbuf, node, SclKeyNum, SclMatOffset, mesh.mAnimation.mSubAnimtion[0].mAnimNode[i]);
                }
            }
    ...
    void makeAnimation() {
    	...
    	for (int i=0; i<mesh.mAnimation.mSubAnimationNum; i++) {
    		...
    		// 计算动画时长
    		float length = (float)animation.mAllAniDurationTime / (float)animation.mTicksPerSecond;
    		...
    		for(DrzAnimationNode track : animation.mAnimNode) {
    			...
    			BoneTrack boneTrack = new BoneTrack(targetBoneIndex, times, translations, rotations, scales);
	...
	private boolean addNodes(final DrzAnimation animation) {	
        ...
        for( int sa = 0; sa < animation.mSubAnimationNum; sa++)//max 128
        {
	        ...
	        for( int i = 0; i < anim.mAnimNodeNum; i++ )
	        {
                ...
                List<DrzAnimationNode> apcNodes = new ArrayList<DrzAnimationNode>();

		        // Now iterate through all nodes in the scene and search for one which has *us* as parent.
		        for( int j = 0; j < anim.mAnimNodeNum; j++ ) 
		        {
			        if( i != j )//do not process it self
			        {
			        	DrzAnimationNode curTrack = anim.mAnimNode[j];
				        if( boneTrack.NodeName.equals(curTrack.ParentNodeName) )
				        {
					        apcNodes.add(curTrack);
					        curTrack.mParentNode = boneTrack;
					        curTrack.hasParent = true;
				        }
			        }
		        }
	
		        //Set all ChildNodes
		        boneTrack.mChildNum = apcNodes.size();
		        if( boneTrack.mChildNum > 0 )
		        {
			        boneTrack.mChildNode = new DrzAnimationNode[boneTrack.mChildNum];
			        for( int j = 0; j < boneTrack.mChildNum; j++ )
			        {
				        boneTrack.mChildNode[j] = apcNodes.get(j);
			        }
		        }

		        //used this, if you use the bone matrices for transformation.
		        //create bone matrices
		        if( boneTrack.hasParent == false)
		        {
                    boneTrack.mTransformation = boneTrack.mOrgFileTransform;
                    boneTrack.mOffsetMatrix = boneTrack.mTransformation;
                    boneTrack.mOffsetMatrix.invert();
                }
		        else
		        {
			        Matrix4f mParentAdjust = new Matrix4f();
                    mParentAdjust = boneTrack.mParentNode.mOrgFileTransform;
                    mParentAdjust.invert();

                    boneTrack.mTransformation = boneTrack.mOrgFileTransform;
                    boneTrack.mTransformation.multLocal(mParentAdjust);

                    Matrix4f ThisNodeInverseMat = new Matrix4f();
                    ThisNodeInverseMat = boneTrack.mTransformation;
                    ThisNodeInverseMat.invert();

                    boneTrack.mOffsetMatrix = boneTrack.mParentNode.mOffsetMatrix;
                    boneTrack.mOffsetMatrix.multLocal(ThisNodeInverseMat);
		        }
        ...
        /*****************************************
        ASE Loader http://read.pudn.com/downloads177/sourcecode/graph/824287/CASEMeshFileLoader.cpp__.htm
        *****************************************/
    }
	 ...
	 * @throws IOException
	 */
	public synchronized boolean loadSmd(InputStream inputStream) throws IOException {
		getByteBuffer(inputStream);
		...
		for (int i = 0; i < meshNum; i++) {
			...
			//ParseMatrix(meshOffset + 240, subMesh.meshMatrix, subMesh.mTMRotation, subMesh.mTMPos, subMesh.mTMScale);
			...
			// Read Faces & MatId
			if (mFaceNum > 0) {
				ParseFaceBlock(meshOffset + 2236 + (mVertexNum * 24), subMesh, mFaceNum);
			}

			// Read tverts && tvfaces
			if (mTVFaceNum > 0) {
				ParseUVFaceVerticesBlock(meshOffset + 2236 + (mVertexNum * 24) + (mFaceNum * 36), subMesh, mTVFaceNum);
			}
...
//			int numunk = mNumRot;
	...
	void createMesh(DrzSubMesh submesh) {
		...
		if (submesh.mFaceList == null) return;
		if (submesh.mFaceList.size() < 1) return;
	 ...
	 * @param indices
	 */
	protected void calculateFaceNormals(Vector3f[] faceNormals, Vector3f[] vertexs, int[] indices) {
	...
	private void ParseMatrix(final int offset, final Matrix4f pMatrix, final Quaternion pTMRotation, final Vector3f pTMPos, final Vector3f pTMScale) {
		buffer.position(offset);
	...
	private boolean ParseVertexData(int offset, int mVertexNum, int mFaceNum, int mTVFaceNum, boolean hasBones, DrzSubMesh submesh) {
		buffer.position(offset);
		...
		for (int k = 0; k < mVertexNum; k++) {
			...
			if (BoneFound == true) {
				// reverting..
				v1 = -((res2 * BoneBuff.s11 * BoneBuff.s2
						- res2 * BoneBuff.s10 * BoneBuff.s3
						- res1 * BoneBuff.s11 * BoneBuff.s6
						+ res1 * BoneBuff.s10 * BoneBuff.s7
						- res3 * BoneBuff.s2 * BoneBuff.s7
						+ res3 * BoneBuff.s3 * BoneBuff.s6
						+ BoneBuff.s12 * BoneBuff.s2 * BoneBuff.s7
						- BoneBuff.s12 * BoneBuff.s3 * BoneBuff.s6
						- BoneBuff.s11 * BoneBuff.s2 * BoneBuff.s8
						+ BoneBuff.s11 * BoneBuff.s4 * BoneBuff.s6
						+ BoneBuff.s10 * BoneBuff.s3 * BoneBuff.s8
						- BoneBuff.s10 * BoneBuff.s4 * BoneBuff.s7) << 8)
						...
						+ BoneBuff.s2 * BoneBuff.s7 * BoneBuff.s9
						- BoneBuff.s3 * BoneBuff.s6 * BoneBuff.s9);
				v2 = ((res2 * BoneBuff.s1 * BoneBuff.s11
						- res1 * BoneBuff.s11 * BoneBuff.s5
						- res3 * BoneBuff.s1 * BoneBuff.s7
						+ res3 * BoneBuff.s3 * BoneBuff.s5
						- res2 * BoneBuff.s3 * BoneBuff.s9
						+ res1 * BoneBuff.s7 * BoneBuff.s9
						...
						+ BoneBuff.s3 * BoneBuff.s8 * BoneBuff.s9
						- BoneBuff.s4 * BoneBuff.s7 * BoneBuff.s9) << 8)
						/ (BoneBuff.s1 * BoneBuff.s11 * BoneBuff.s6
						- BoneBuff.s11 * BoneBuff.s2 * BoneBuff.s5
						- BoneBuff.s1 * BoneBuff.s10 * BoneBuff.s7
						+ BoneBuff.s10 * BoneBuff.s3 * BoneBuff.s5
						+ BoneBuff.s2 * BoneBuff.s7 * BoneBuff.s9
						- BoneBuff.s3 * BoneBuff.s6 * BoneBuff.s9);
				v3 = -((res2 * BoneBuff.s1 * BoneBuff.s10
						- res1 * BoneBuff.s10 * BoneBuff.s5
						- res3 * BoneBuff.s1 * BoneBuff.s6
						+ res3 * BoneBuff.s2 * BoneBuff.s5
						- res2 * BoneBuff.s2 * BoneBuff.s9
						+ res1 * BoneBuff.s6 * BoneBuff.s9
						...
						+ BoneBuff.s2 * BoneBuff.s8 * BoneBuff.s9
						- BoneBuff.s4 * BoneBuff.s6 * BoneBuff.s9) << 8)
						/ (BoneBuff.s1 * BoneBuff.s11 * BoneBuff.s6
						- BoneBuff.s11 * BoneBuff.s2 * BoneBuff.s5
						- BoneBuff.s1 * BoneBuff.s10 * BoneBuff.s7
						+ BoneBuff.s10 * BoneBuff.s3 * BoneBuff.s5
				...
				Vector3f vert = new Vector3f(-y, z, -x);
				submesh.mVertexList.add(vert);

			} else {
				System.out.println("Vertex read error! No valid mesh matrix found. BoneName=[" + strBoneName + "] hasBone=" + hasBones);
				return false;
			}
	...
	private void ParseFaceBlock(final int offset, final DrzSubMesh submesh, final int mFaceNum) {
		buffer.position(offset);
		...
		for (int k = 0; k < mFaceNum; k++) {
			buffer.position(buf);
			submesh.mFaceList.add(new DrzFaceTextureId(getShort(), getShort(), getShort(), getShort()));
			// next face
			buf += 36;
		}
	}

	private void ParseUVFaceVerticesBlock(final int offset, final DrzSubMesh submesh, final int UVFaceNum) {
		buffer.position(offset);
		...
		for (int i = 0; i < UVFaceNum; i++) {
			submesh.mUvVertexList1.add(new Vector2f(getFloat(uvoff + 0), 1-getFloat(uvoff + 12)));
			submesh.mUvVertexList1.add(new Vector2f(getFloat(uvoff + 4), 1-getFloat(uvoff + 16)));
			submesh.mUvVertexList1.add(new Vector2f(getFloat(uvoff + 8), 1-getFloat(uvoff + 20)));
	...
	private void ParseRotationAniBlock(final int _offset, final DrzAnimationNode ppAniNode,
			final int RotKeyNum, final int RotMatOffset, final DrzAnimationNode AseExportRootNode) {
		...
		for (int i = RotMatOffset; i < RotMatOffset + RotKeyNum; i++) {
			...
			// TODO　四元数要考虑超过90度的情况,还要考虑max坐标系和jme坐标系的不同处
			...
			// We need non reversed and stand alone Quaternion for ase files. So i create an export list, insteed reverse the dx quats
	...
	private void ParsePositionAniBlock(final int _offset, final DrzAnimationNode ppAniNode,
			final int PosKeyNum, final int PosMatOffset, final DrzAnimationNode AseExportRootNode) {
		int buf = _offset;
		buf += PosMatOffset * 16;
	...
	private void ParseScaleAniBlock(final int _offset, final DrzAnimationNode ppAniNode,
			final int SclKeyNum, final int SclMatOffset, final DrzAnimationNode AseExportRootNode) {
		int buf = _offset;
		buf += SclMatOffset * 16;
		for (int i = SclMatOffset; i < SclMatOffset + SclKeyNum; i++) {
			...
			sclKey.mValue = new Vector3f(z, y, x);
