 ...
 * @param <T>
 */
public abstract class ResourceInitilize<T> {
	...
	private FilenameFilter filter = new FilenameFilter() {
		...
		public boolean accept(File dir, String name) {
			return ResourceInitilize.this.accept(dir, name);
		}
	};
			...
			for (int i=0; i<files.length; i++) {
				...
				if (file.isFile()) {
					...
					try {
						...
						// 解析文件
						t = decode();
						if (t instanceof CharMonsterInfo) {
							((CharMonsterInfo)t).File = file.getName();
						}
