...
public class TestInxLoader {
	...
	public static void main(String[] args) {
		...
		// 读取地图的smd文件
		// MODELINFO model = (MODELINFO) assetManager.loadAsset(new SmdKey("char/npc/arad/arad.inx", SMDTYPE.INX));
		MODELINFO model = (MODELINFO) assetManager.loadAsset(new SmdKey("char/monster/death_knight/death_knight.inx", SMDTYPE.MODELINFO));
		...
		for(int i=10; i<model.MotionCount; i++) {
			...
			if (motion.State == 0) {
				...
				log.debug(i + ":" + name + " Key1:" + motion.MotionKeyWord_1 + " Key2:" + motion.MotionKeyWord_2 + " Start:" + motion.StartFrame + " End:" + motion.EndFrame + " Repeat:" + motion.Repeat + " KeyCode:" + motion.KeyCode + " Frames:" + motion.MotionFrame);
			}
		}
